...
import com.goat.userservice.constants.FriendApplicationStatusEnum;
import com.goat.userservice.constants.KafkaTopicConstant;
import com.goat.userservice.mapper.ApplyFriendMapper;
...
public class ApplyFriendServiceImpl extends ServiceImpl<ApplyFriendMapper, ApplyFriend> implements ApplyFriendService {
     ...
     * @return 申请列表
     */
    public IPage<ApplyFriend> getReceivedRequests(Long userId, int pageNum, int pageSize) {
        ThrowUtils.throwIf(userId == null, ErrorCode.PARAMS_ERROR, "用户ID不能为空");
    ...
    private List<ApplyFriendDTO> mapApplyFriendsToDTO(List<ApplyFriend> applyFriends, Long userId) {
        ...
        if (!targetUserIds.isEmpty() && userService != null) {
    ...
    protected ModifyFriendApplicationResponse handleRejectApplication(Long receiverId, List<Long> senderIds) {
        ThrowUtils.throwIf(senderIds.size() != 1, ErrorCode.PARAMS_ERROR, "拒绝状态只能包含一个接收者");
