...
| 当前限制 | 单设备 token/路由模型；系统通知补拉尚未完整实现，部分旧接口仍通过 userId 参数识别用户 |
...
6. 使用 MinIO 中的默认群头像；
7. 群主可在群资料页上传 JPG、PNG 或 WebP 自定义头像。

会话详情、会话设置、群成员分页、群资料修改和基础群管理公开接口已经补齐，并校验当前登录用户的会话成员身份和角色。页面不得依赖 `/api/user/**` 下的内部校验接口。
...
5. V0.2 已完成群成员分页、资料修改、角色管理、成员退出/移除、群主转让、解散，以及会话置顶、免打扰和隐藏接口，下一步补群昵称；
