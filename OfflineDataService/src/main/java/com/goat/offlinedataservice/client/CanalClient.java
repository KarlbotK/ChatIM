...
public class CanalClient implements CommandLineRunner {
    ...
    private void storeMessageToRedis(MessageResponse messageResponse){
        ...
        try {
            double score = messageResponse.getCreatedTime();
    ...
    private MessageResponse buildMessageFromMap(Map<String, String> map) {
        ...
        messageResponse.setCreatedTime(parseCreatedTime(map.get("created_time")));
    ...
    private long parseCreatedTime(String createdTime) {
        try {
            return DATE_FORMATTER.get().parse(createdTime).getTime();
        } catch (Exception e) {
            throw new IllegalArgumentException("无法解析消息创建时间: " + createdTime, e);
        }
    }
}
