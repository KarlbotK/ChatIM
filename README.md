...
> 当前版本适合本地开发、产品演示和接口联调。可靠消息 ACK、发送幂等、结果查询、服务端离线同步游标、会话摘要、会话详情、已读位置、服务端未读数和群成员分页已经接入；群资料修改及成员管理仍在下一轮计划中。
...
- 群资料和成员分页已经接入；群名称、公告、头像修改，以及角色管理、退出和解散接口尚未实现。
