package com.goat.redpacketservice.service.impl;
...
import com.goat.redpacketservice.client.UserServiceClient;
...
import com.goat.redpacketservice.model.vo.*;
...
import com.goat.redpacketservice.util.RedPacketAlgorithm;
...
public class RedPacketServiceImpl implements RedPacketService {
    ...
    public void handleRedPacketExpiration(Long redPacketId) {
        ...
        // 如果红包已经领完或已过期，不需要处理
        if (redPacket.getStatus() != RedPacketConstant.STATUS_NOT_COMPLETED) {
            log.info("红包状态不是未领取完，跳过过期处理。红包ID: {}, 状态: {}", redPacketId, redPacket.getStatus());
            return;
        }
        ...
        // 4. 更新红包状态为已过期
        redPacket.setStatus(RedPacketConstant.STATUS_EXPIRED);
        redPacket.setUpdatedTime(new Date());
