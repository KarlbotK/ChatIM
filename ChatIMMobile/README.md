...
后端目前还没有公开的会话摘要、群详情、群成员列表和完整的 WebSocket 发送确认协议。真实模式会从联系人接口加载好友与申请，并支持创建群聊和邀请好友，但刷新后的跨设备群聊列表仍需等待公开会话接口。`?demo=1` 中的聊天发送会模拟从 `sending` 到 `sent`，用于验证界面与交互，不代表服务端已经完成消息持久化确认。
