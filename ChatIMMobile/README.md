...
后端目前还没有公开的会话摘要、群详情、群成员列表和完整的 WebSocket 发送确认协议。真实模式会从联系人接口加载好友与申请，支持创建群聊、邀请好友以及把图片上传到预签名 MinIO 地址；图片上传完成后会显示“结果待确认”，直到后续接入 WebSocket 回推。刷新后的跨设备会话列表仍需等待公开会话接口。`?demo=1` 中的发送状态用于验证界面与交互，不代表服务端已经完成消息持久化确认。
